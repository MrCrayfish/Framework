 ...
 */
public final class FrameworkEditBox extends AbstractWidget implements ContainerEventHandler
{
    ...
    private static final WidgetTextures DEFAULT_SPRITES = new WidgetTextures(
        FrameworkTexture.nineSlice(FRAMEWORK_WIDGETS_LOCATION, 0, 0, 100, 20, Border.of(1)),
        FrameworkTexture.nineSlice(FRAMEWORK_WIDGETS_LOCATION, 0, 0, 100, 20, Border.of(1))
    );
    ...
    private final @Nullable WidgetTextures background;
    ...
    private FrameworkEditBox(int x, int y, int width, int height, Function<FrameworkEditBox, Icon> icon, Padding padding, int spacing, @Nullable WidgetTextures background, Border border, String text, @Nullable String suggestion, @Nullable Component hint, @Nullable Consumer<String> callback, @Nullable Predicate<String> valueFilter, @Nullable BiFunction<String, Integer, FormattedCharSequence> styleFormatter, @Nullable Supplier<Boolean> activeSupplier, @Nullable Integer maxTextLength, boolean clearOnRightClick, @Nullable Integer iconWidthOverride)
    {
        ...
        if(this.icon != null)
        {
            ...
            this.iconWidget = helper.addChild(new CenteredSpriteWidget(iconWidth, 0, this.icon),
                LayoutSettings.defaults().paddingTop(border.top() + padding.top()).paddingBottom(border.bottom() + padding.bottom()).paddingLeft(padding.left() + border.left())
            );
        }
        else
        {
        ...
        if(this.icon == null) {
            settings = settings.paddingLeft(border.left() + padding.left());
        }
        this.editBox = helper.addChild(new Impl(this), settings);
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        if(this.background != null)
        {
            ...
            FrameworkTexture background = this.background.get(this.editBox.isActive(), this.editBox.isFocused());
            background.draw(graphics, this.getX(), this.getY(), this.getWidth(), this.getHeight());
    ...
    public static class Builder
    {
        ...
        private @Nullable WidgetTextures background = DEFAULT_SPRITES;
        private String text = "";
         ...
         *
         * @param background a {@link WidgetTextures} containing the texture resources
         * @return this {@link Builder} for method chaining
         */
        public Builder setBackground(@Nullable WidgetTextures background)
        {
            this.background = background;
            return this;
        }
