...
public final class FrameworkSelect<T> extends AbstractWidget
{
    ...
    public static final ResourceLocation DEFAULT_DROPDOWN_BACKGROUND = Utils.rl("widget/select/dropdown_background");
    ...
    private final @Nullable ResourceLocation dropdownBackground;
    ...
    private FrameworkSelect(int x, int y, int width, int height, Class<T> type, Function<T, Component> objectToLabel, List<T> values, boolean searchable, @Nullable Consumer<T> callback, @Nullable Anchor preferredAnchor, @Nullable WidgetSprites texture, Function<FrameworkSelect<T>, @Nullable Icon> iconFunction, @Nullable ResourceLocation dropdownBackground, Padding dropdownPadding, int dropdownMinWidth, int visibleListItems, @Nullable Supplier<Boolean> active, @Nullable Function<FrameworkSelect<T>, Tooltip> tooltip, int tooltipDelay, int tooltipOptions, @Nullable Consumer<FrameworkSelectionList.Builder> buildOptionListListener, @Nullable Consumer<FrameworkEditBox.Builder> buildSearchFieldListener, @Nullable Consumer<Dropdown.Builder> buildDropdownListener)
    {
        ...
        var searchFieldBuilder = FrameworkEditBox.builder()
            ...
            .setPadding(2, 0, 2, 0)
            .setIcon(ResourceLocation.withDefaultNamespace("icon/search"), 12, 12)
            .setCallback(s -> {
    ...
    protected void renderWidget(GuiGraphics graphics, int mouseX, int mouseY, float partialTick)
    {
        ...
        if(this.texture != null)
        {
            ...
            graphics.setColor(1, 1, 1, this.active ? 1.0F : 0.75F);
            graphics.blitSprite(this.texture.get(this.active, this.isHoveredOrFocused() && this.active), this.getX(), this.getY(), this.getWidth(), this.getHeight());
        ...
        if(this.iconFunction != null)
        {
            ...
            if(icon != null)
            {
                ...
                icon.draw(graphics, iconX, iconY, partialTick);
                textEnd = iconX;
            }
        }
    ...
    private class OptionItem extends FrameworkSelectionList.Item
    {
        ...
        {
            graphics.drawCenteredString(Minecraft.getInstance().font, this.label, x + width / 2, y + 4, 0xFFFFFFFF);
        }

        @Override
        protected void renderBackground(FrameworkSelectionList.@Nullable ItemSprites sprites, GuiGraphics graphics, int index, int x, int y, int width, int height, int mouseX, int mouseY, boolean hovered, boolean selected)
        {
            selected = (FrameworkSelect.this.selected == this) || hovered;
            super.renderBackground(sprites, graphics, index, x, y, width, height, mouseX, mouseY, hovered, selected);
        }
    }
    ...
    public static class Builder<T>
    {
        ...
        private @Nullable ResourceLocation dropdownBackground = DEFAULT_DROPDOWN_BACKGROUND;
         ...
         * dropdown builder.
         *
         * @param sprite a {@link ResourceLocation} to a nine-slice sprite, or {@code null} for a full transparent background
         * @return this {@link Builder} instance for method chaining
         */
        public Builder<T> setDropdownBackground(@Nullable ResourceLocation sprite)
        {
